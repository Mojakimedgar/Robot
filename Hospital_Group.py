...
MQTT_PORT = 1883
MQTT_TOPIC = "robotdb/hospital_group"
...
def fetch_and_publish_device_data():
    try:
        query = "SELECT Hospital_Group_id, Group_Name FROM hospital_group;"
        cursor.execute(query)
        rows = cursor.fetchall()

        # Create a table to display the fetched data
        table = PrettyTable()
        table.field_names = ["Hospital_Group_id", " Group_Name"]

        for row in rows:
            if len(row) >= 2:
                payload = {
                    "Hospital_Group_id": row[0],
                    "Group_Name": row[1],
...
def monitor_database():
    last_row_count = 0
    while True:
        query = "SELECT COUNT(*) FROM hospital_group;"
        cursor.execute(query)
